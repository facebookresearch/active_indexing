...
```
This will create a new folder `DISC21/ref_10k` containing the 10k reference images used as queries in the dev set, and a `DISC21/ref_990k` folder containing the remaining 990k reference images. (Note that only symlinks are created, the images are not duplicated.)
...
```
This will save the features in a torch `fts.pt` file in the `--output` folder, as well as a `filenames.txt` file containing the list of filenames corresponding to the features.
